...
struct PaulHeckel<Element: Identifiable & Equatable> {
    ...
    static func computeDiff(from old: [Element], to new: [Element]) -> [Operation] {
        // 1) 심볼 테이블 생성: 키별 발생 빈도와 인덱스 목록 수집
        let symbol = buildSymbolTable(old: old, new: new)

        // 2) 왼쪽/오른쪽 매칭 버퍼(동일 키의 고유 매칭 라벨) 준비
        var oldLabels = Array(repeating: -1, count: old.count)
        var newLabels = Array(repeating: -1, count: new.count)

        // 3) 1차 매칭: 새 배열에서 한 번만 등장하는 키를 기준으로 동일 위치 매칭
        findUniqueMatches(old: old, new: new, symbol: symbol, oldLabels: &oldLabels, newLabels: &newLabels)

        // 4) 전파 매칭(좌→우): 인접한 동일 키를 연쇄적으로 매칭
        propagateMatchesForward(old: old, new: new, oldLabels: &oldLabels, newLabels: &newLabels)
        // 5) 전파 매칭(우→좌)
        propagateMatchesBackward(old: old, new: new, oldLabels: &oldLabels, newLabels: &newLabels)

        // 6) 연산 방출: 삭제, 삽입, 이동, 업데이트 순으로 계산
        return emitOperations(old: old, new: new, oldLabels: oldLabels, newLabels: newLabels)
    }
    ...
    private static func findUniqueMatches(old: [Element], new: [Element], symbol: [AnyHashable: SymbolEntry], oldLabels: inout [Int], newLabels: inout [Int]) {
        for (_, entry) in symbol {
            ...
            // 같은 키가 old에도 1개 이상 있으면, 가장 앞의 것을 우선 매칭(Heckel의 기본 규칙)
            let oldIndex = entry.oldIndices.first!
    ...
    private static func propagateMatchesForward(old: [Element], new: [Element], oldLabels: inout [Int], newLabels: inout [Int]) {
        ...
        while i < old.count && j < new.count {
            // 이미 매칭된 페어를 찾는다
            while i < old.count && oldLabels[i] == -1 { i += 1 }
