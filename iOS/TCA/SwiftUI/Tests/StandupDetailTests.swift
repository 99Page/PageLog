...
final class StandupDetailTests: XCTestCase {
    func testEdit() async throws {
        ...
        let standupFormAction = StandupFormFeature.Action.set(\.$standup, standup)
        let standupDesinationAction = StandupDetailFeature.Destination.Action.editStandup(standupFormAction)
        await store.send(.destination(.presented(standupDesinationAction))) 
