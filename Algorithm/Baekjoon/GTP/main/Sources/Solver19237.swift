...
struct Solver19237 {
    ...
    init() {
        ...
        let input: [Int] = Reader.readArray() // input을 먼저 읽어와 저장
        ...
        let map: [[Int]] = Reader.readGrid(rowSize)
        ...
        let initialDirectionInput: [Int] = Reader.readArray()
    ...
    struct SharkPriority {
        ...
        init() {
            ...
            let upPriorityValue: [Int] = Reader.readArray()
            upPriority = upPriorityValue.map({ Direction(value: $0) })
            
            let downPriorityValue: [Int] = Reader.readArray()
            downPriority = downPriorityValue.map({ Direction(value: $0) })
            
            let leftPriorityValue: [Int] = Reader.readArray()
            leftPriority = leftPriorityValue.map({ Direction(value: $0) })
            
            let rightPriorityValue: [Int] = Reader.readArray()
