...
struct Solver1103 {
    ...
    mutating func solve() {
        let initialIsVisited: [[Bool]] = Array(repeating: Array(repeating: false, count: colSize), count: rowSize)
        let initialCoordinate = Coordinate(row: 0, col: 0, distance: 0)
        var result = move(coordinate: initialCoordinate, isVisited: initialIsVisited)
...
//        print(mem)
    ...
    mutating func move(coordinate: Coordinate, isVisited: [[Bool]]) -> Int {
        ...
        for nextCoordinate in coordinate.findValidNextPositions(rowSize: rowSize, colSize: colSize, distance: distance) {
            var isVisitedNew = isVisited
            
//            print("[\(coordinate.row)][\(coordinate.col)] -> [\(nextCoordinate.row)][\(nextCoordinate.col)]")
            
            if isVisitedNew[nextCoordinate.row][nextCoordinate.col] {
                mem[row][col] = maxMove
            } else {
                isVisitedNew[nextCoordinate.row][nextCoordinate.col] = true
                let maxVisitCount = move(coordinate: nextCoordinate, isVisited: isVisitedNew)
                let currentMaxCount = maxVisitCount == .max ? .max : maxVisitCount + 1
                isVisitedNew[nextCoordinate.row][nextCoordinate.col] = false
                
                mem[row][col] = max(currentMaxCount, mem[row][col])
            }
        }
        
        return max(mem[row][col], 1)
    }
